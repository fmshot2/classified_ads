...
# Your agent registration was successful!
## <strong>These are your details for backup</strong>

<p><strong>Full Name: </strong> {{ $agent_name ? $agent_name : 'Name not provided!' }}</p>
<p><strong>Email: </strong> {{ $agent_email ? $agent_email : 'Name not provided!' }}</p>
<p><strong>Password: </strong> {{ $password ? $password : 'Name not provided!' }}</p>
<p><strong>Agent Code: </strong> {{ $code ? $code : 'Agent Code not provided!' }}</p>

@component('mail::button', ['url' => route('save_agent_Login')])
Login Now
@endcomponent
